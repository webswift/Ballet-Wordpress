...
jQuery(document).ready(function() {
		window.send_to_editor = function(html) {
		 ...
		 if (formfield=='img') {
		 	document.forms["form-playlist-all_in_one_bannerWithPlaylist-"+the_i].img.value=imgurl;
		 } else {
			 document.forms["form-playlist-all_in_one_bannerWithPlaylist-"+the_i].thumbnail.value=imgurl;
		 }
		 //alert (the_i); 	
...
<div class="wrap">
  ...
  <div id="all_in_one_bannerWithPlaylist_updating_witness"><img src="<?php echo plugins_url('images/ajax-loader.gif', dirname(__FILE__))?>" /> Updating...</div>
  
  
<div style="text-align:center; padding:0px 0px 20px 0px;"><img src="<?php echo plugins_url('images/icons/add_icon.gif', dirname(__FILE__))?>" alt="add" align="absmiddle" /> <a href="?page=all_in_one_bannerWithPlaylist_Playlist&xmlf=add_playlist_record">Add new</a></div>
<div style="text-align:left; padding:10px 0px 10px 14px;">#Initial Order</div>


<ul id="all_in_one_bannerWithPlaylist_sortable">
	<?php foreach ( $result as $row ) 
	{
		$row=all_in_one_bannerWithPlaylist_unstrip_array($row); ?>
	<li class="ui-state-default cursor_move" id="<?php echo $row['id']?>">#<?php echo $row['ord']?> ---  <img src="<?php echo $row['img']?>" height="30" align="absmiddle" id="top_image_<?php echo $row['id']?>" /><div class="toogle-btn-closed" id="toogle-btn<?php echo $row['ord']?>" onclick="mytoggle('toggleable<?php echo $row['ord']?>','toogle-btn<?php echo $row['ord']?>');"></div><div class="options"><a href="javascript: void(0);" onclick="all_in_one_bannerWithPlaylist_delete_entire_record(<?php echo $row['id']?>,<?php echo $row['ord']?>);">Delete</a></div>
	<div class="toggleable" id="toggleable<?php echo $row['ord']?>">
    <form method="POST" enctype="multipart/form-data" id="form-playlist-all_in_one_bannerWithPlaylist-<?php echo $row['ord']?>">
		...
		<table width="100%" cellspacing="0" class="wp-list-table widefat fixed pages" style="background-color:#FFFFFF;">
          ...
          <tr>
            ...
            <td align="left" valign="middle"><input name="img" type="text" id="img" size="100" value="<?php echo stripslashes($row['img']);?>" />
              <input name="upload_img_button_<?php echo $row['ord']?>" type="button" id="upload_img_button_bannerWithPlaylist_<?php echo $row['ord']?>" value="Change Image" />
              <br />
...
<tr>
          ...
          <tr>
            ...
            <td align="left" valign="middle"><input name="thumbnail" type="text" id="thumbnail" size="100" value="<?php echo stripslashes($row['thumbnail'])?>" />
              <input name="upload_thumbnail_button_<?php echo $row['ord']?>" type="button" id="upload_thumbnail_button_bannerWithPlaylist_<?php echo $row['ord']?>" value="Change Thumbnail" />
              <br />
              Enter an URL or upload an image<br />
              <br />
  ...
  Elegant: 90px × 90px
		  ...
		  <tr>
		    ...
		    <td align="left" valign="middle"><select name="data-transition" id="data-transition">
		      <option value="" <?php echo (($row['data-transition']=='')?'selected="selected"':'')?>>select...</option>
		      <option value="random" <?php echo (($row['data-transition']=='random')?'selected="selected"':'')?>>random</option>
		      <option value="asynchronousDroppingStripes" <?php echo (($row['data-transition']=='asynchronousDroppingStripes')?'selected="selected"':'')?>>asynchronousDroppingStripes</option>
		      <option value="bottomTopDroppingReverseStripes" <?php echo (($row['data-transition']=='bottomTopDroppingReverseStripes')?'selected="selected"':'')?>>bottomTopDroppingReverseStripes</option>
		      <option value="bottomTopDroppingStripes" <?php echo (($row['data-transition']=='bottomTopDroppingStripes')?'selected="selected"':'')?>>bottomTopDroppingStripes</option>
		      <option value="fade" <?php echo (($row['data-transition']=='fade')?'selected="selected"':'')?>>fade</option>
		      <option value="leftRightFadingReverseStripes" <?php echo (($row['data-transition']=='leftRightFadingReverseStripes')?'selected="selected"':'')?>>leftRightFadingReverseStripes</option>
		      <option value="leftRightFadingStripes" <?php echo (($row['data-transition']=='leftRightFadingStripes')?'selected="selected"':'')?>>leftRightFadingStripes</option>
		      <option value="randomBlocks" <?php echo (($row['data-transition']=='randomBlocks')?'selected="selected"':'')?>>randomBlocks</option>
		      <option value="slideFromBottom" <?php echo (($row['data-transition']=='slideFromBottom')?'selected="selected"':'')?>>slideFromBottom</option>
		      <option value="slideFromLeft" <?php echo (($row['data-transition']=='slideFromLeft')?'selected="selected"':'')?>>slideFromLeft</option>
		      <option value="slideFromTop" <?php echo (($row['data-transition']=='slideFromTop')?'selected="selected"':'')?>>slideFromTop</option>
		      <option value="topBottomDiagonalBlocks" <?php echo (($row['data-transition']=='topBottomDiagonalBlocks')?'selected="selected"':'')?>>topBottomDiagonalBlocks</option>
		      <option value="topBottomDiagonalReverseBlocks" <?php echo (($row['data-transition']=='topBottomDiagonalReverseBlocks')?'selected="selected"':'')?>>topBottomDiagonalReverseBlocks</option>
		      <option value="topBottomDroppingReverseStripes" <?php echo (($row['data-transition']=='topBottomDroppingReverseStripes')?'selected="selected"':'')?>>topBottomDroppingReverseStripes</option>
		      <option value="topBottomDroppingStripe" <?php echo (($row['data-transition']=='topBottomDroppingStripe')?'selected="selected"':'')?>>topBottomDroppingStripe</option>
		      </select></td>
		    </tr>
		  <tr>
		    <td align="right" valign="top" class="row-title">Playlist Title</td>
		    <td align="left" valign="top"><input name="data-title" type="text" size="60" id="data-title" value="<?php echo stripslashes($row['data-title']);?>"/></td>
		    </tr>
		  <tr>
		    <td align="right" valign="top" class="row-title">Playlist Description</td>
		    <td align="left" valign="top"><textarea name="data-desc" id="data-desc" cols="45" rows="5"><?php echo stripslashes($row['data-desc']);?></textarea></td>
		    </tr>
		  <tr>
		    <td align="right" valign="top" class="row-title">&nbsp;</td>
		  ...
		  <tr>
		    ...
		    <td align="left" valign="middle"><input name="texts<?php echo $row['ord']?>" id="texts<?php echo $row['ord']?>" type="button" class="button-primary" value="Open Texts Editor" onclick="all_in_one_bannerWithPlaylist_open_dialog(<?php echo $row['ord']?>)"></td>
...
<div id="dialog<?php echo $row['ord']?>" title="Manage Texts Over the Photo" style="display:none; padding:0; margin:0;">
	<div id="photo_div<?php echo $row['id']?>" style="padding:50px 0 0px 50px;"><img src="<?php echo $row['img']?>" />
    <?php
    	$safe_sql=$wpdb->prepare( "SELECT * FROM (".$wpdb->prefix ."all_in_one_bannerWithPlaylist_texts) WHERE photoid = %d ORDER BY id",$row['id'] );
...
<script>
		jQuery("#draggable<?php echo $row_text['id']?>").draggable( { 
			...
			drag: function(event, ui) { 
				jQuery("#data-initial-left<?php echo $row_text['id']?>").val(all_in_one_bannerWithPlaylist_process_val(jQuery(this).css('left'),'left'));
				jQuery("#data-initial-top<?php echo $row_text['id']?>").val(all_in_one_bannerWithPlaylist_process_val(jQuery(this).css('top'),'top'));
			}
		});
</script>    
  ...
  <tr>
	...
	<tr>
	<td colspan="8"><div class="delete_text" onclick="all_in_one_bannerWithPlaylist_delete_text_line(<?php echo $row_text['id']?>)">&nbsp;</div></td>
	</tr>  
</table>
    </div>
    ...
    <p><input name="all_in_one_bannerWithPlaylist_add_text_line<?php echo $row['ord']?>" id="all_in_one_bannerWithPlaylist_add_text_line<?php echo $row['ord']?>" type="button" class="button-primary" value="Add New Text Line" onclick="all_in_one_bannerWithPlaylist_add_text_line(<?php echo $row['id']?>)"></p>
