...
class Envira_Dynamic_Gallery_Shortcode {
	...
	*/
	public function get_folder_images( $dynamic_data, $folder, $data ) {
	    ...
	    foreach ( (array) $images as $i => $image_filename ) {
			...
			$suffix_retina 	= '-' . $instance->get_config( 'crop_width', $data ) * 2 . 'x' . $instance->get_config( 'crop_height', $data )  * 2 . ( $instance->get_config( 'crop', $data ) ? '_c ' : '' ) . '.' . $ext;
			...
			$suffix_retina 	= '-' . $instance->get_config( 'mobile_width', $data ) * 2 . 'x' . $instance->get_config( 'mobile_height', $data )  * 2 . ( $instance->get_config( 'crop', $data ) ? '_c ' : '' ) . '.' . $ext;
			if ( strpos( $image_filename, $suffix ) !== false ||  strpos( $image_filename, $suffix_retina ) !== false ) {
				continue;
			}
			...
			$suffix_retina 	= '-' . $instance->get_config( 'thumbnails_width', $data )  * 2 . 'x' . $instance->get_config( 'thumbnails_height', $data )  * 2 . '_c.' . $ext;
			if ( strpos( $image_filename, $suffix ) !== false ||  strpos( $image_filename, $suffix_retina ) !== false ) {
				continue;
			}
	...
	*/
	function override_gallery( $html, $atts ) {
	    ...
	    $result = do_shortcode( '[envira-gallery-dynamic id="custom-gallery-' . $dynamic_i . '" ' . trim( $output_args ) . ']' );
	...
	function shortcode( $atts ) {
	    ...
	    $shortcode = '[envira-gallery dynamic="' . $id . '" ' . trim( $output_args ) . ']';
