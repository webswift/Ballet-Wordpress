    ...
    public function images_display( $post ) {
		...
		<div id="envira-defaults">
        	<p class="envira-intro">
		        ...
		        switch ( get_post_type ( $post ) ) {
			        case 'envira':
                        _e( 'Default Gallery Settings', 'envira-defaults' );
                        break;
			        case 'envira_album':
                        _e( 'Default Album Settings', 'envira-defaults' );
                        break;
		        }
	        	?>
            </p>
            <p>
                ...
                switch ( get_post_type ( $post ) ) {
                    case 'envira':
                        _e( 'This gallery and its settings will be used as defaults for any new gallery you create on this site. Any of these settings can be overwritten on an individual gallery basis via template tag arguments or shortcode parameters.', 'envira-defaults' );
                        break;
                    case 'envira_album':
                        _e( 'This album and its settings will be used as defaults for any new albums you create on this site. Any of these settings can be overwritten on an individual album basis via template tag arguments or shortcode parameters.', 'envira-defaults' );
                        break;
                }
                ?>
            </p>
            ...
            <div class="envira-video-help">
                <iframe src="https://www.youtube.com/embed/oQrbPdtOnOk/?rel=0" width="600" height="338" frameborder="0" allowfullscreen></iframe>
            </div>
            ...
            <p>
                <a href="http://enviragallery.com/docs/defaults-addon/" title="Click here for Defaults Addon documentation." target="_blank" class="button button-primary envira-button-primary">
                    <?php _e( 'Click here for Defaults Addon Documentation', 'envira-defaults' ); ?>
                </a>
	        </p>
    	</div>
    	<?php
