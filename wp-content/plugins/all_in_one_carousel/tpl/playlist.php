...
<div class="wrap">
	<div id="lbg_logo">
			<h2>Playlist for carousel: <span style="color:#FF0000; font-weight:bold;"><?php echo $_SESSION['xname']?> - ID #<?php echo $_SESSION['xid']?></span></h2>
 	</div>
  <div id="all_in_one_carousel_updating_witness"><img src="<?php echo plugins_url('images/ajax-loader.gif', dirname(__FILE__))?>" /> Updating...</div>
  
  
<div style="text-align:center; padding:0px 0px 20px 0px;"><img src="<?php echo plugins_url('images/icons/add_icon.gif', dirname(__FILE__))?>" alt="add" align="absmiddle" /> <a href="?page=all_in_one_carousel_Playlist&xmlf=add_playlist_record">Add new</a></div>
		...
		<table width="100%" cellspacing="0" class="wp-list-table widefat fixed pages" style="background-color:#FFFFFF;">
          ...
          <tr>
            ...
            <td align="left" valign="middle"><input name="img" type="text" id="img" size="100" value="<?php echo stripslashes($row['img']);?>" />
              <input name="upload_img_button_<?php echo $row['ord']?>" type="button" id="upload_img_button_carousel_<?php echo $row['ord']?>" value="Change Image" />
              <br />
...
.allinone_carousel.charming .contentHolderUnit </blockquote>
		...
		<tr>
          ...
          <tr>
            ...
            <td align="left" valign="middle"><input name="thumbnail" type="text" id="thumbnail" size="100" value="<?php echo stripslashes($row['thumbnail'])?>" />
              <input name="upload_thumbnail_button_<?php echo $row['ord']?>" type="button" id="upload_thumbnail_button_carousel_<?php echo $row['ord']?>" value="Change Thumbnail" />
              <br />
              Enter an URL or upload an image<br />
              <br />
        ...
        <td align="left" valign="middle"><img src="<?php echo $row['thumbnail']?>" name="thumbnail_<?php echo $row['ord']?>" id="thumbnail_<?php echo $row['ord']?>" /></td>
          ...
          <tr>
            ...
            <td align="left" valign="top"><textarea name="data-title" id="data-title" cols="45" rows="5"><?php echo stripslashes($row['data-title']);?></textarea></td>
          </tr>
		  <tr>
...
<!--		  <tr>
		    ...
		    <td align="left" valign="middle"><input name="texts<?php echo $row['ord']?>" id="texts<?php echo $row['ord']?>" type="button" class="button-primary" value="Open Texts Editor" onclick="all_in_one_carousel_open_dialog(<?php echo $row['ord']?>)"></td>
...
<script>
		jQuery("#draggable<?php echo $row_text['id']?>").draggable( { 
			...
			drag: function(event, ui) { 
				jQuery("#data-initial-left<?php echo $row_text['id']?>").val(all_in_one_carousel_process_val(jQuery(this).css('left'),'left'));
				jQuery("#data-initial-top<?php echo $row_text['id']?>").val(all_in_one_carousel_process_val(jQuery(this).css('top'),'top'));
			}
		});
</script>    
  ...
  <tr>
	...
	<tr>
	<td colspan="8"><div class="delete_text" onclick="all_in_one_carousel_delete_text_line(<?php echo $row_text['id']?>)">&nbsp;</div></td>
	</tr>  
</table>
    </div>
    ...
    <p><input name="all_in_one_carousel_add_text_line<?php echo $row['ord']?>" id="all_in_one_carousel_add_text_line<?php echo $row['ord']?>" type="button" class="button-primary" value="Add New Text Line" onclick="all_in_one_carousel_add_text_line(<?php echo $row['id']?>)"></p>
