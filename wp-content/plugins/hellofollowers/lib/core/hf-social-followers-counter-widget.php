...
add_action( 'widgets_init' , create_function( '' , 'return register_widget( "HFSocialFollowersCounterWidget" );' ) );
...
class HFSocialFollowersCounterWidget extends WP_Widget {
	...
	public function __construct() {
		...
		$options = array( 'description' => __( 'Widet to display your social follower counters' , 'hellofollowers' ) );
		parent::__construct( false , __( 'Hello Followers Counter' , 'hellofollowers' ) , $options );
	}
	...
	private function generate_select_field($field, $title, $description, $value, $list_of_values) {
		...
		if (!empty($description)) {
			$output .= '<span style="font-weight: 300; font-size: 0.9em"><br /><em>'. __( $description , 'hellofollowers' ).'</em></span>';
		}
		$output .= '</p>';
