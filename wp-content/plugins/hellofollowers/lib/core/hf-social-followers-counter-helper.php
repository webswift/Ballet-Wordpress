...
class HFSocialFollowersCounterHelper {
	 ...
	 * @since 3.4
	 */
	public static function default_options_structure($apply_defaults = false, $custom_defaults) {
		$structure = array();
	...
	public static function options_structure() {
		...
		$mailpoet_lists = array_merge( array( array( 'list_id' => 'all', 'name' => __(' Total Subscribers', 'hellofollowers' ))), $mailpoet_lists);
		$mailpoet_lists = array_merge( array( array( 'list_id' => '', 'name' => __(' ', 'hellofollowers' ))), $mailpoet_lists);
	...
	public static function create_default_options_from_structure($options) {
		...
		foreach ($structure as $network => $data) {
			$base_network_option_id = "hf_".$network."_";
		...
		$options['hf_update'] = 1440;
		$options['hf_format'] = 'short';
	...
	public static function mymail_get_lists() {
		...
		if( class_exists( 'mymail' ) ) {
			...
			$mymail_lists[0]->name 	= __(' Total Subscribers', 'hellofollowers' );
			$mymail_lists 			= array_merge($mymail_lists , $mymail_lists_items);
	
			$mymail_lists 			= array();
			$mymail_lists[0] 		= new stdClass();
			$mymail_lists[0]->ID 	= '';
			$mymail_lists[0]->name 	= __('', 'hellofollowers' );
			$mymail_lists 			= array_merge($mymail_lists , $mymail_lists_items);
