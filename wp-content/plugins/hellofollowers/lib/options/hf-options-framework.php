...
class HelloFollowersOptionsFramework {
	...
	public static function draw_options_field($option, $custom = false, $user_settings = array()) {		
		...
		$settings_group = "hf_options";
		$is_fans_counter = false;
		$option_value = isset($hf_options[$id]) ? $hf_options[$id] : '';
		...
		switch ($type) {
			...
			case "checkbox_list_sortable":
				$ordered_values = isset($hf_options[$id.'_order']) ? $hf_options[$id.'_order'] : array();
			...
			case "image_checkbox":
				...
				//self::draw_image_checkbox_field($id, $listOfValues, 'hf_options', array("checkbox_option2" => true));
				self::draw_options_row_end();
				break;
			case "image_radio":
				self::draw_options_row_start($title, $description, $recommended);
				self::draw_image_radio_field($id, $listOfValues, $settings_group, $option_value);
				//self::draw_image_radio_field($id, $listOfValues, 'hf_options', 'checkbox_option1');
				self::draw_options_row_end();
				break;
			case "func":
				self::draw_options_row_start($title, $description, $recommended);
	...
	public static function draw_settings_panel_start($title) {
		print '<div class="hf-admin-options-panel">';
		
		printf('<div class="hf-admin-options-panel-title">%1$s</div>', $title);
		print '<div class="hf-admin-options-panel-content">';
	}
	...
	public static function draw_settings_panel_end($description = '', $recommended = '') {
		...
		print '</div>';
		printf('<div class="hf-admin-options-panel-desc">%1$s</div>', $description);
		print '</div>';
	}
	...
	public static function draw_heading($title, $level = '1', $submenu_link = '') {
		...
		if ($submenu_link != '') {
			$submenu_link = sprintf('id="hf-submenu-%1%s"', $submenu_link);
		}
		
		if (empty($submenu_link) && $level != '1') {
			self::$headings_count++;
			$navigation_id = "hf-internal-".self::$headings_count;
	...
	public static function draw_simplesort_field($field, $listOfValues, $settings_group = 'hf_options', $value = '') {
		if ($value == '') {
			$value = $listOfValues; // initialize default values
		}
		
		printf('<ul class="hf_sortable" id="hf-sortable-%1$s">', $field);
	...
	public static function draw_image_checkbox_field($field, $listOfValues, $settings_group = 'hf_options', $value = '') {
		...
		echo '<div class="hf_image_checkbox_container hf_image_checkbox_container_'.$field.'">';
		foreach ( $listOfValues as $singleValueCode => $singleValue ) {
			...
			if ($label != '') {
				$label = sprintf('<div class="hf_checkbox_label">%1$s</div>', $label);
			}	
			
			printf('<div class="hf_checkbox"><div class="hf_image_checkbox%6$s" data-field="%8$s_%3$s">
					<span class="checkbox-image"><img src="%1$s/%2$s"/></span>
					<span class="checkbox-state"><i class="fa fa-lg fa-check-circle"></i></span>
					<input type="checkbox" id="hf_options_%8$s_%3$s" name="%4$s[%8$s][]" value="%3$s"%7$s/>					
					</div>%5$s</div>', ESSB3_PLUGIN_URL, $singleValue['image'], $singleValueCode, $settings_group, $label, $active_state, $active_element, $field);
		}	
		echo '</div>';
	}

	public static function draw_image_radio_field($field, $listOfValues, $settings_group = 'hf_options', $value = '') {		
		$exist_user_value = false;
		...
		echo '<div class="hf_image_radio_container hf_image_radio_container_'.$field.'">';
		$position = 1;
		foreach ( $listOfValues as $singleValueCode => $singleValue ) {
			$label = isset($singleValue['label']) ? $singleValue['label'] : '';
			...
			if ($label != '') {
				$label = sprintf('<div class="hf_radio_label">%1$s</div>', $label);
			}
				
			printf('<div class="hf_radio"><div class="hf_image_radio%8$s" data-field="%6$s_%7$s">
					<span class="checkbox-image"><img src="%1$s/%2$s"/></span>
					<span class="checkbox-state"><i class="fa fa-lg fa-check-circle"></i></span>
					<input type="radio" id="hf_options_%6$s_%7$s" name="%4$s[%6$s]" value="%3$s"%9$s/>
	...
	public static function draw_textarea_field($field, $settings_group = 'hf_options', $value = '') {
		$value = esc_textarea ( stripslashes($value));
		printf('<textarea id="hf_options_%1$s" name="%2$s[%1$s]" class="input-element stretched" rows="5">%3$s</textarea>', $field, $settings_group, $value);
	}

	public static function draw_wpeditor_field($field, $settings_group = 'hf_options', $value = '') {
		$value =  ( stripslashes($value));

		
		//printf('<div id="wpeditor_%1$s"></div>', $field);
		printf('<textarea id="hf_options_%1$s" name="%2$s[%1$s]" class="input-element stretched hf-hidden-element" rows="5">%3$s</textarea>', $field, $settings_group, $value);
		$editor_options = array('textarea_name' => $settings_group.'['.$field.']');
		wp_editor($value, sprintf('wpeditor_%1$s', $field), $editor_options);
	}
	
	public static function draw_editor_field($field, $settings_group = 'hf_options', $value = '', $mode = 'htmlmixed') {
		$value = esc_textarea ( stripslashes($value));
		printf('<textarea id="hf_options_%1$s" name="%2$s[%1$s]" class="input-element stretched is-code-editor" rows="5" data-function-activate="activate__%1$s">%3$s</textarea>', $field, $settings_group, $value);
		printf('<script>
				function activate__%1$s() {
				console.log("actiting %1$s");
    var editor_%1$s = CodeMirror.fromTextArea(document.getElementById("hf_options_%1$s"), {
      lineNumbers: true,
      mode: "%2$s",
				lineWrapping: true,      
      matchBrackets: true,
	...
	public static function draw_switch_field($field, $settings_group = 'hf_options', $value = '', $on_text = '', $off_text = '') {
		if ($settings_group == '') { $settings_group = "hf_options"; }
		...
		printf('<div class="hf-switch">
				<label class="cb-enable%6$s"><span>%4$s</span></label>
				<label class="cb-disable%7$s"><span>%5$s</span></label>
				<input id="hf_options_%1$s" type="checkbox" name="%2$s[%1$s]" value="true" class="input-element checkbox" %3$s />
				</div>', $field, $settings_group, $is_checked, $on_text, $off_text, $on_switch, $off_switch);
	}
	
	public static function draw_input_field($field_id, $fullwidth = false, $settings_group = 'hf_options', $value = '', $icon = '', $class = '', $icon_position = '') {
		
		$value = esc_attr ( stripslashes($value));
		if ($icon != '' && $icon_position != 'right') {
			printf('<span class="hf-input-prefix-icon"><i class="fa %1$s"></i></span>', $icon);
		}
		
		echo '<input id="hf_options_' . $field_id . '" type="text" name="' . $settings_group . '[' . $field_id . ']" value="' . $value . '" class="input-element' . ($fullwidth ? ' stretched' : '') . ' '.$class.'" />';

		if ($icon != '' && $icon_position == 'right') {
			printf('<span class="hf-input-suffix-icon"><i class="fa %1$s"></i></span>', $icon);
		}
	}

	public static function draw_fileselect_field($field_id, $settings_group = 'hf_options', $value = '', $icon = '', $class = '') {
		if ($icon != '') {
			printf('<span class="hf-input-prefix-icon"><i class="fa %1$s"></i></span>', $icon);
		}
		...
		echo '<input id="hf_options_' . $field_id . '" type="text" name="' . $settings_group . '[' . $field_id . ']" value="' . $value . '" class="input-element small-stretched '.$class.'" />';
		echo '<a href="#" class="button" id="hf_fileselect_' . $field_id . '">'.__('Select File', ESSB3_TEXT_DOMAIN).'</a>';
		?>
		...
		jQuery(document).ready(function($){
		    ...
		    $('#hf_fileselect_<?php echo $field_id; ?>').click(function(e) {
		        ...
		        custom_uploader.on('select', function() {
		            ...
		            $('#hf_options_<?php echo $field_id; ?>').val(attachment.url);
		        });
	...
	public static function draw_select_field($field, $listOfValues, $simpleList = false, $group = 'hf_options', $value = '') {
		
		echo '<select name="' . $group . '[' . $field . ']" class="input-element" id="hf_options_' . $field . '">';
	...
	public static function draw_select2_field($field, $listOfValues, $simpleList = false, $group = 'hf_options', $value = '', $select2_options = array()) {
		...
		echo '<select name="' . $group . '[' . $field . ']" class="input-element '.($multiple ? 'stretched' : "").'" id="hf_options_' . $field . '" '.($multiple ? 'multiple="multiple"' : "").'>';
		...
		jQuery("#hf_options_' . $field . '").select2({
  placeholder: "'.$placeholder.'",
  allowClear: '.$allow_clear_state.'
});});</script>';
	}
	
	public static function draw_checkbox_field($field, $group = 'hf_options', $value = '') {
		$is_checked = ($value == 'true') ? ' checked="checked"' : '';
		echo '<input id="hf_options_' . $field . '" type="checkbox" name="' . $group . '[' . $field . ']" value="true" ' . $is_checked . ' />';
	}

	public static function draw_checkbox_list_field($field, $listOfValues, $group = 'hf_options', $value = '') {
		if (!is_array($value)) { $value = array(); }
		
		foreach ($listOfValues as $key => $text) {
			$is_checked = in_array($key, $value) ? ' checked="checked"' : '';
			echo '<span class="hf_checkbox_list_item"><input id="hf_options_' . $field . '" type="checkbox" name="' . $group . '[' . $field . '][]" value="'.$key.'" ' . $is_checked . ' />'.$text.'</span>';
		}
	}
	
	public static function draw_checkbox_list_sortable_field($field, $listOfValues, $group = 'hf_options', $value = '') {
		if (!is_array($value)) {
			$value = array();
		}
	
		/*
		 * printf('<ul class="essb_sortable" id="hf-sortable-%1$s">', $field);
		...
		foreach ($listOfValues as $key => $text) {
			$is_checked = in_array($key, $value) ? ' checked="checked"' : '';
			echo '<li><i class="fa fa-bars" style="margin-right: 3px; margin-left: 3px;"></i><span class="hf_checkbox_list_item"><input id="hf_options_' . $field . '" type="checkbox" name="' . $group . '[' . $field . '][]" value="'.$key.'" ' . $is_checked . ' />'.$text.'<input type="hidden" name="' . $group . '[' . $field . '_order][]" value="'.$key.'|'.$text.'"/></span></li>';
		}
		echo '</ul>';
	}
	
	public static function draw_color_field($field, $group = 'hf_options', $value = '') {
		
		$value = stripslashes ( $value );
		
		echo '<input id="hf_options_' . $field . '" type="text" name="' . $group . '[' . $field . ']" value="' . $value . '" class="input-element stretched" data-default-color="' . $value . '" />';
		
		array_push ( self::$color_fields, 'hf_options_' . $field );
	}
