...
jQuery(document).ready(function() {
		window.send_to_editor = function(html) {
		 ...
		 if (formfield=='img') {
		 	document.forms["form-playlist-all_in_one_thumbnailsBanner-"+the_i].img.value=imgurl;
		 } else {
			 document.forms["form-playlist-all_in_one_thumbnailsBanner-"+the_i].thumbnail.value=imgurl;
		 }
		 //alert (the_i); 	
...
<div class="wrap">
  ...
  <div id="all_in_one_thumbnailsBanner_updating_witness"><img src="<?php echo plugins_url('images/ajax-loader.gif', dirname(__FILE__))?>" /> Updating...</div>
  
  
<div style="text-align:center; padding:0px 0px 20px 0px;"><img src="<?php echo plugins_url('images/icons/add_icon.gif', dirname(__FILE__))?>" alt="add" align="absmiddle" /> <a href="?page=all_in_one_thumbnailsBanner_Playlist&xmlf=add_playlist_record">Add new</a></div>
<div style="text-align:left; padding:10px 0px 10px 14px;">#Initial Order</div>


<ul id="all_in_one_thumbnailsBanner_sortable">
	<?php foreach ( $result as $row ) 
	{
		$row=all_in_one_thumbnailsBanner_unstrip_array($row); ?>
	<li class="ui-state-default cursor_move" id="<?php echo $row['id']?>">#<?php echo $row['ord']?> ---  <img src="<?php echo $row['img']?>" height="30" align="absmiddle" id="top_image_<?php echo $row['id']?>" /><div class="toogle-btn-closed" id="toogle-btn<?php echo $row['ord']?>" onclick="mytoggle('toggleable<?php echo $row['ord']?>','toogle-btn<?php echo $row['ord']?>');"></div><div class="options"><a href="javascript: void(0);" onclick="all_in_one_thumbnailsBanner_delete_entire_record(<?php echo $row['id']?>,<?php echo $row['ord']?>);">Delete</a></div>
	<div class="toggleable" id="toggleable<?php echo $row['ord']?>">
    <form method="POST" enctype="multipart/form-data" id="form-playlist-all_in_one_thumbnailsBanner-<?php echo $row['ord']?>">
		...
		<table width="100%" cellspacing="0" class="wp-list-table widefat fixed pages" style="background-color:#FFFFFF;">
          ...
          <tr>
            ...
            <td align="left" valign="middle"><input name="img" type="text" id="img" size="100" value="<?php echo stripslashes($row['img']);?>" />
              <input name="upload_img_button_<?php echo $row['ord']?>" type="button" id="upload_img_button_thumbnailsBanner_<?php echo $row['ord']?>" value="Change Image" />
              <br />
        ...
        <td align="left" valign="middle"><img src="<?php echo $row['img']?>" width="300" id="img_<?php echo $row['ord']?>" /></td>
          ...
          <tr>
            ...
            <td align="left" valign="middle"><input name="thumbnail" type="text" id="thumbnail" size="100" value="<?php echo stripslashes($row['thumbnail'])?>" />
              <input name="upload_thumbnail_button_<?php echo $row['ord']?>" type="button" id="upload_thumbnail_button_thumbnailsBanner_<?php echo $row['ord']?>" value="Change Thumbnail" />
              <br />
              Enter an URL or upload an image<br />
	        <br />
	        Recommended size for each skin: 110px × 65px</td>
            </tr>
          <tr>
        <td align="right" valign="top" class="row-title">&nbsp;</td>
        <td align="left" valign="middle"><img src="<?php echo $row['thumbnail']?>" name="thumbnail_<?php echo $row['ord']?>" id="thumbnail_<?php echo $row['ord']?>" /></td>
		  ...
		  <tr>
		    ...
		    <td align="left" valign="middle"><input name="texts<?php echo $row['ord']?>" id="texts<?php echo $row['ord']?>" type="button" class="button-primary" value="Open Texts Editor" onclick="all_in_one_thumbnailsBanner_open_dialog(<?php echo $row['ord']?>)"></td>
...
<div id="dialog<?php echo $row['ord']?>" title="Manage Texts Over the Photo" style="display:none; padding:0; margin:0;">
    ...
    <?php
    	$safe_sql=$wpdb->prepare( "SELECT * FROM (".$wpdb->prefix ."all_in_one_thumbnailsBanner_texts) WHERE photoid = %d ORDER BY id",$row['id'] );
...
<script>
		jQuery("#draggable<?php echo $row_text['id']?>").draggable( { 
			...
			drag: function(event, ui) { 
				jQuery("#data-initial-left<?php echo $row_text['id']?>").val(all_in_one_thumbnailsBanner_process_val(jQuery(this).css('left'),'left'));
				jQuery("#data-initial-top<?php echo $row_text['id']?>").val(all_in_one_thumbnailsBanner_process_val(jQuery(this).css('top'),'top'));
			}
		});
</script>    
  ...
  <tr>
	...
	<tr>
	<td colspan="8"><div class="delete_text" onclick="all_in_one_thumbnailsBanner_delete_text_line(<?php echo $row_text['id']?>)">&nbsp;</div></td>
	</tr>  
</table>
    </div>
    ...
    <p><input name="all_in_one_thumbnailsBanner_add_text_line<?php echo $row['ord']?>" id="all_in_one_thumbnailsBanner_add_text_line<?php echo $row['ord']?>" type="button" class="button-primary" value="Add New Text Line" onclick="all_in_one_thumbnailsBanner_add_text_line(<?php echo $row['id']?>)"></p>
