...
class ESSBSocialFollowersCounterWidget extends WP_Widget {
	...
	public function __construct() {
		...
		$options = array( 'description' => __( 'Social Followers Counter' , ESSB3_TEXT_DOMAIN ) );
		parent::__construct( false , __( 'Easy Social Share Buttons: Followers Counter' , ESSB3_TEXT_DOMAIN ) , $options );
	}
	...
	private function generate_select_field($field, $title, $description, $value, $list_of_values) {
		...
		if (!empty($description)) {
			$output .= '<span style="font-weight: 300; font-size: 0.9em"><br /><em>'. __( $description , ESSB3_TEXT_DOMAIN ).'</em></span>';
		}
		$output .= '</p>';
