...
if (!function_exists('essb_subscribe_form_design1')) {
	function essb_subscribe_form_design1($salt, $is_widget = false) {
		global $essb_options;
		
		$subscribe_mc_namefield = ESSBOptionValuesHelper::options_bool_value($essb_options, 'subscribe_mc_namefield');
		...
		$subscribe_mc_name = ESSBOptionValuesHelper::options_value($essb_options, 'subscribe_mc_name');
		...
		$output = '<div class="essb-subscribe-form-content essb-subscribe-from-design1'.($is_widget ? " essb-subscribe-form-inwidget" :"").'">';
		...
		$output .= '<input class="submit" name="submit" type="submit" value="'.$subscribe_mc_button.'" onclick="essb_ajax_subscribe(\''.$salt.'\', event);">';
		$output .= '</form>';
