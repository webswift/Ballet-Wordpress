...
var essb_flyin_onscroll = function() {
	...
	if (!flyinTriggerEnd) {
		...
		var element = jQuery('.essb_break_scroll');
		if (!element.length) { return; }
		var top = jQuery('.essb_break_scroll').offset().top - parseFloat(jQuery('.essb_break_scroll').css('marginTop').replace(/auto/, 0));
...
var essb_flyin_show = function() {
	...
	if (popHideOnClose == "1" || popHideOnCloseAll == "1") {
		...
		if (popHideOnClose == "1") {
			...
			var cookieSet = essbFlyInGetCookie(cookie_name);
			if (cookieSet == "yes") { return; }
			essbFlyInSetCookie(cookie_name, "yes", 7);
		}
	...
	if (Number(popAutoCloseAfter) && Number(popAutoCloseAfter) > 0) {
		shortly = new Date(); 
		var user_message = jQuery('#essb_settings_flyin_user_autoclose').text();
		if (user_message == '') {
			user_message = "Window will automatically close in ";
		}
	    shortly.setSeconds(shortly.getSeconds() + Number(popAutoCloseAfter)); 
	    jQuery('.essb_flyin_counter_text').countdown({until: shortly, onExpiry: essb_flyin_close, "layout" : user_message + " {sn} {sl}"});
	}
	
	jQuery(".essb-flyin").css( { width: base_width+'px'});
	
	jQuery(".essb-flyin").fadeIn(400);
...
function watchCountdown (periods) { 
    ...
    jQuery('.essb_flyin_counter_text').text(user_message + 
        periods[6] + ' sec'); 
}

var essb_flyin_close = function() {

	jQuery(".essb-flyin").fadeOut(200);
}
...
jQuery(document).ready(function($){
	...
	if ('true' == get_url_parameter('essb_popup')) {
		if (element.hasClass("essb-flyin-oncomment")) {
			essb_flyin_show();
			return;
		}
	}
	...
	if (popOnPercent != '' || popOnEnd == "1") {
		...
		$(window).scroll(essb_flyin_onscroll);
		return;
	}
	...
	if (popAfter && typeof(popAfter) != "undefined") {
		if (popAfter != '' && Number(popAfter)) {
			shortly1 = new Date(); 
		    shortly1.setSeconds(shortly1.getSeconds() + Number(popAfter)); 
		    $('#essb_settings_flyafter_counter').countdown({until: shortly1, onExpiry: essb_flyin_show});
